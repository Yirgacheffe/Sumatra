...
/**
 * Some common character string operations are encapsulated in this class. 
 * All methods are static��It needn't to create  this class's instance,In order to escape creating this class's instance,
 * Method of the structure should be declared as private. 
 * 
 * @since 1.0
 */
...
{  
   ...
   * @return   
   *        While Array is carved up,if "source" is null,the array equal to String[0],
   *        else if "delim" is null,comma by the way of the compartmental character string.
   * @since 0.1
   */
   ...
   *        Compartmental character string of the  word.
   * @return 
   *        While Array is carved up,if "source" is null,return the array which uses it as the only argument.  
   * @since 0.1
   */
  ...
  /**
   * Repace the content of the string with the "values". If the content contains expression than it can't repace.
   * @param prefix
   ...
   *          If the prefix is empty than the "%" looked as the prefix;
   *          If the source or values is null than return "source";
   *          If the length of the values is great than the parameter's count than 
   *          the redundant values is ignored;
   *          If the length of the values is less than the parameter's count than 
  ...
  /**
   * Replace the variant(the prefix of the variant is '%') of the string with the contant of the values
   * 
   ...
   * @return 
   *          The string which has been replaced.
   * @since 0.1
   */
  ...
  /**
   * 
   * Whether the strings contains the appointed string.
   *           Neglect capital or small letter.
   * @param strings
   *         Original strings.
   * @param string
   *         Appointed string. 
  ...
  /**
   * Function to get random ID,the default value is a random string of 15 bits length. 
   * @retuen 
   *         A random string of 15 bits length. 
   */
  ...
  /**
   * Convert the string (yes or no, 1 or 0, y or n) to corresponding value: true or false.
   * @param theString
  ...
  /**
   * Convert the number 1 to 26 to corresponding capital letter,
   *          for example:1 correspond to A, 2 correspond to B,26 correspond to Z.
   * @param number
  ...
  /**
   * Format an int to string which length is appointed, if the length is less than appointed length,add 0 ahead 
   * of the int.
   * @param i
   *          an int to foamat.
   * @param len
  ...
  /**
   * Format a string to another string which length is appointed, 
   * if the formatted string's length is less than appointed length,add appointed character at the end of it.
   * @param str
   *        string to format.  
   * @param len
   ...
   * @return 
   *         String which has been formatted.
   */        
   ...
   * @return
   */
  public static String replaceFirst(String src, String srcSubStr, String descStr)
  {

    if ((null == src || "".equals(src))
        || (null == srcSubStr || "".equals(srcSubStr)) || descStr == null
        || descStr.equals(srcSubStr))
      return src;
  ...
  public static String decode(String str){
    try {
		return str==null?"":new String(str.getBytes("ISO8859_1"),"UTF-8");
